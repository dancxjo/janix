...
APPS := init init_debug clock window_demo compositor hello_world geographer debug_alloc taskman debug_input_logger debug_input_events watch_test
...
$(IMAGE_NAME).iso: limine/limine kernel user drivers icons assets
	...
	for app in clock window_demo compositor hello_world geographer debug_alloc taskman debug_input_logger debug_input_events watch_test; do \
...
ifeq ($(ENABLE_geographer),1)
	...
	echo '    module_path: boot():/boot/user/watch_test' >> iso_root/boot/limine/limine.conf
	echo '    module_cmdline: program=watch_test' >> iso_root/boot/limine/limine.conf
ifeq ($(ENABLE_PLATARO_ICONS),1)
