...
pub fn load_ani(data: &[u8]) -> Option<CursorAsset> {
    ...
    if &data[8..12] != b"ACON" { return None; }
        ...
        match &id {
            b"anih" => {
                if chunk_data.len() >= 36 {
                    default_rate = u32::from_le_bytes([chunk_data[24], chunk_data[25], chunk_data[26], chunk_data[27]]);
                }
            }
            b"rate" => {
                ...
                for i in 0..count {
                    ...
                    if offset + 4 <= chunk_data.len() {
                        let jiffies = u32::from_le_bytes([chunk_data[offset], chunk_data[offset + 1], chunk_data[offset + 2], chunk_data[offset + 3]]);
            ...
            b"seq " => {
                ...
                for i in 0..count {
                    let offset = i * 4;
                    if offset + 4 <= chunk_data.len() {
                        let idx = u32::from_le_bytes([chunk_data[offset], chunk_data[offset + 1], chunk_data[offset + 2], chunk_data[offset + 3]]);
                        sequence.push(idx);
                    }
                }
            }
    ...
    Some(CursorAsset { frames, delays_ms, sequence, is_animated: true })
}

struct RiffParser<'a> { data: &'a [u8], offset: usize }
...
impl<'a> RiffParser<'a> {
    ...
    fn next_chunk(&mut self) -> Option<([u8; 4], &'a [u8])> {
        ...
        let size = u32::from_le_bytes([self.data[self.offset + 4], self.data[self.offset + 5], self.data[self.offset + 6], self.data[self.offset + 7]]) as usize;
