...
pub fn main() {
    ...
    // Ensure we have a graph to put devices
    let devices_graph = match thing_find("graph.pci") {
        Some(d) => d,
        None => {
            // thing_create takes (kind, parent). If we pass ThingId(0), it is root?
            // Wait, graph.rs says: thing_create(kind, parent).
            // We want graph.pci to be in graph.root.
            let root = thing_find("graph.root").unwrap_or(ThingId(0));
            // Ensure root is valid if possible.
            
            let p = thing_create(symbol_intern("kind.Graph"), root);
            thing_register_name(p, "graph.pci");
            // If parent=0 was passed effectively, or if we need explicit contains relationship:
            if root.0 != 0 {
                relationship_create(symbol_intern("predicate.contains"), root, p);
            }
            p
        }
    };
    ...
    #[cfg(target_arch = "x86_64")]
    {
        ...
        enumerate::scan_segment(&access, 0, 0, 255, devices_graph);
    }
