//! Entry point for AArch64 architecture.
//!
...
unsafe extern "C" fn rust_main() -> ! {
    ...
    if let Some(resp) = hhdm_req {
        ...
        if let Some(mm) = memmap_req {
            for entry in mm.entries() {
                if entry.entry_type == boot::loader::EntryType::USABLE {
        ...
        let (phys, size) = heap_region.expect("BOOT: Heap not found");
        let virt = phys + offset;
        heap::init_kernel_heap(virt as usize, size as usize);

        early_log::log_heap_init(phys, virt, size);

        // 3. Paging & Platform
        paging::init(offset);
        paging::map_device_region(0x09000000, 4096); // UART
        paging::map_device_region(0x08000000, 4096); // GICD
        paging::map_device_region(0x08010000, 4096); // GICC

        Bridge::init_platform(offset);
        crate::bridge::set_uart_base(0x09000000 + offset);
    ...
    boot_screen::set_blit_hook(simd::neon_blit);

    let bridge = Bridge;
    // (Existing setup code assumed, just changing the collection line)
    ...
    *KERNEL.lock() = Some(k);
    ...
    // Ingest Modules & Spawn Loaded moved above.
...
fn page_fault_hook(
    frame: &mut crate::bringup::interrupts::trap::TrapFrame,
    fault_addr: u64,
    esr: u64,
) {
    unsafe {
