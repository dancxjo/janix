...
unsafe fn write_reg(index: u16, val: u16) {
    asm!("out dx, ax", in("dx") VBE_DISPI_IOPORT_INDEX, in("ax") index);
    asm!("out dx, ax", in("dx") VBE_DISPI_IOPORT_DATA, in("ax") val);
}

unsafe fn read_reg(index: u16) -> u16 {
    let mut val: u16;
    asm!("out dx, ax", in("dx") VBE_DISPI_IOPORT_INDEX, in("ax") index);
    asm!("in ax, dx", out("ax") val, in("dx") VBE_DISPI_IOPORT_DATA);
    val
}

pub unsafe fn init(k: &mut Kernel<Bridge>, pci_devices: &[PciDeviceBody]) -> Option<(u64, u64)> {
    ...
    crate::print_hex(id as u64); // print_hex is in bridge_x86_64
    k.bridge.log(")\n");
    ...
    if lfb_phys == 0 {
        k.bridge.log("DRIVER(qemu_vga): Could not find LFB address via PCI. Aborting.\n");
        return None;
    }
    ...
    use models::builtins::ids::{THING_DISPLAY_FRAMEBUFFER_KIND, THING_HAS_DEVICE_KIND, THING_BOOT_ROOT};
...
pub unsafe fn set_mode(width: u16, height: u16, bpp: u16) {
    ...
    write_reg(VBE_DISPI_INDEX_VIRT_HEIGHT, height * 2);
