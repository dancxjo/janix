...
pub fn init<B: HardwareBridge>(dev: &PciDeviceBody, k: &mut Kernel<B>) -> u32 {
    ...
    // 2. Map ABAR
    let hhdm = k.bridge.hhdm_offset();
    if hhdm == 0 {
        k.bridge.log("AHCI: HHDM not set!\n");
        return 0;
    }
...
unsafe fn init_port(bridge: &impl HardwareBridge, port: &mut HbaPort, port_no: usize, hhdm: u64) {
    ...
    bridge.log("AHCI: Port DMA Initialized. Starting...\n");
...
pub unsafe fn read_sector_yielding<F: Fn()>(
    ...
    if port.sig == 0xEB140101 {
        read_atapi_sector_yielding(bridge, port, port_idx, lba, buf, hhdm, yield_fn)
    } else {
        false
    }
}
