...
impl AddressSpace {
    ...
    fn probe_user_page(&self, virt: u64, perms: MapPerms) -> bool {
        let l0 = phys_to_virt(self.pgd) as *const u64;
        let l0_idx = ((virt >> 39) & 0x1ff) as usize;
...
fn entry_permits(entry: u64, perms: MapPerms) -> bool {
    if entry & PTE_V == 0 || entry & PTE_P == 0 {
        return false;
    }
    if entry & PTE_PLV_USER != PTE_PLV_USER {
        return false;
    }
    if perms.contains(MapPerms::READ) && entry & PTE_NR != 0 {
        return false;
    }
    if perms.contains(MapPerms::WRITE) && entry & PTE_W == 0 {
        return false;
    }
    if perms.contains(MapPerms::EXEC) && entry & PTE_NX != 0 {
        return false;
    }
    true
}
