...
extern "x86-interrupt" fn double_fault_handler(sf: InterruptStackFrame, ec: u64) -> ! {
    ...
    write(b"\n");
    write(b"RIP: ");
    write_hex(sf.instruction_pointer.as_u64());
    write(b"\n");
    write(b"CS:  ");
    write_hex(sf.code_segment as u64);
    write(b"\n");
    write(b"RFLAGS: ");
    write_hex(sf.cpu_flags);
    write(b"\n");
    write(b"RSP: ");
    write_hex(sf.stack_pointer.as_u64());
    write(b"\n");
    write(b"SS:  ");
    write_hex(sf.stack_segment as u64);
    write(b"\n");
    write(b"==========================================\n");
    panic!("DOUBLE FAULT");
}
...
extern "x86-interrupt" fn page_fault_handler(sf: InterruptStackFrame, ec: PageFaultErrorCode) {
    ...
    write(b"\n");
    write(b"  Cause: ");
    if ec.contains(PageFaultErrorCode::PROTECTION_VIOLATION) {
        write(b"Protection\n");
    } else {
        write(b"Not present\n");
    }
    write(b"  Access: ");
    if ec.contains(PageFaultErrorCode::CAUSED_BY_WRITE) {
        write(b"WRITE\n");
    } else {
        write(b"READ\n");
    }
