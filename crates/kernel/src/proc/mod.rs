...
pub fn spawn_kernel_module(module: &crate::boot::ModuleInfo) -> Result<(), ()> {
    ...
    if stack_mem.is_null() { return Err(()); }
    
    // Zero init stack
    unsafe { core::ptr::write_bytes(stack_mem, 0, stack_size as usize); }

    let stack_phys = crate::machine::machine().virt_to_phys(stack_mem as u64);
    
    // Map stack
    address_space.map(stack_base, stack_phys, stack_size as usize, MapPerms::READ | MapPerms::WRITE | MapPerms::USER).map_err(|_| ())?;
...
fn load_elf(data: &[u8], _as: &crate::memory::space::AddressSpace) -> Result<u64, ()> {
    ...
    for i in 0..ph_num {
        ...
        if p_flags & 1 != 0 { perms |= MapPerms::EXEC; }
        if p_flags & 2 != 0 { perms |= MapPerms::WRITE; }
        if p_flags & 4 != 0 { perms |= MapPerms::READ; }
        ...
        if mem.is_null() { return Err(()); }
        
        unsafe { core::ptr::write_bytes(mem, 0, len); }
