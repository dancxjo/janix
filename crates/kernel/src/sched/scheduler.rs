...
pub struct Scheduler<B: CpuBridge> {
    pub threads: Vec<Option<Thread<B::Context, B::FpuState>>>,
...
impl<B: CpuBridge> Default for Scheduler<B> {
    fn default() -> Self {
        Self::new()
    }
}

impl<B: CpuBridge> Scheduler<B> {
    ...
    pub fn spawn(
        ...
        let thread = Thread {
            ...
            fpu_context: B::FpuState::default(),
            started: false,
            thing_id: None,
            sleep_event_id: None,
            sleep_until_ns: 0,
    ...
    pub fn tick(&mut self, bridge: &B, current_context: &mut ThreadContext<B::Context>)
    where
        B::Context: Copy,
    {
        ...
        if let Some(tid) = self.current {
            ...
            if let Some(Some(thread)) = self.threads.get_mut(tid.0 as usize - 1) {
                ...
                bridge.save_fpu(&mut thread.fpu_context);
                ...
                if thread.state == ThreadState::Running {
                    thread.state = ThreadState::Runnable;
                    self.run_queue.push_back(tid);
                    bridge.log("SCHED: Re-queued Running\n");
                } else if thread.state == ThreadState::Runnable {
                    self.run_queue.push_back(tid);
                    bridge.log("SCHED: Re-queued Runnable\n");
                } else {
                    bridge.log("SCHED: Thread not re-queued. dropped.\n");
                }
            }
        }
        ...
        if let Some(next_tid) = self.pick_next() {
            bridge.log("SCHED: Picked next\n");
            self.current = Some(next_tid);
            if let Some(Some(thread)) = self.threads.get_mut(next_tid.0 as usize - 1) {
                thread.state = ThreadState::Running;

                bridge.log("SCHED: Switch to ");
                bridge.log(&thread.name);
                bridge.log("\n");

                bridge.set_kernel_stack(thread.kernel_stack_top);
                bridge.restore_fpu(&thread.fpu_context);
