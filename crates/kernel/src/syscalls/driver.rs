...
/// # Safety
/// Caller must ensure `out_ptr` is valid for writing `out_len` bytes.
pub unsafe fn sys_driver_wait<B: HardwareBridge>(
    kernel: &mut Kernel<B>,
    out_ptr: *mut u8,
    out_len: usize,
) -> SysRet {
    ...
    if let Some(event) = crate::input::try_pop_event(&kernel.bridge) {
        // Serialize
        // SAFETY: Caller guarantees pointer validity via unsafe contract.
...
/// # Safety
/// Caller must ensure `ptr` is valid for reading `len` bytes.
pub unsafe fn sys_driver_publish<B: HardwareBridge>(
    kernel: &mut Kernel<B>,
    ptr: *const u8,
    len: usize,
) -> SysRet {
