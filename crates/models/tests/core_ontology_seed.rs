...
use models::schema::{LinkRule, SchemaBody};
use models::*;
...
fn test_core_ontology_seed() {
    ...
    let process_kind_body: KindBody = process_kind_thing
        .body
        .decode()
        .expect("Decode Process KindBody");
    ...
    let process_schema_body: SchemaBody = process_schema_thing
        .body
        .decode()
        .expect("Decode Process SchemaBody");
    ...
    let has_owns = process_schema_body
        .link_rules
        .iter()
        .any(|r| r.predicate_kind == THING_OWNS_KIND);
    let has_cap = process_schema_body
        .link_rules
        .iter()
        .any(|r| r.predicate_kind == THING_HAS_CAP_KIND);
    ...
    let mount_schema_body: SchemaBody = mount_schema_thing
        .body
        .decode()
        .expect("Decode Mount SchemaBody");

    // Verify MOUNTS min=1 max=1
    let mounts_rule = mount_schema_body
        .link_rules
        .iter()
        .find(|r| r.predicate_kind == THING_MOUNTS_KIND)
    ...
    let kind_schema_body: SchemaBody = kind_schema_thing
        .body
        .decode()
        .expect("Decode Kind SchemaBody");
    let has_schema_rule = kind_schema_body
        .link_rules
        .iter()
        .find(|r| r.predicate_kind == THING_HAS_SCHEMA_KIND)
