...
fn main(boot_fd: usize) -> ! {
                        ...
                        if offset + arg1_len <= buf.len() {
                            if let Ok(s) = core::str::from_utf8(&buf[offset..offset + arg1_len]) {
                                if let Ok(val) = s.parse::<usize>() {
                                    boot_fd = val;
                                    debug!("display_bootfb: Recovered boot_fd {} from argv[1]", boot_fd);
                                }
                            }
                        }
                    }
    ...
    if boot_fd != 0 {
        ...
        if let Ok(resp) = stem::syscall::vm_map(&req) {
            ...
            debug!("display_bootfb: Bootstrap handles: req_read={}, resp_write={}, svc={}, id={}", 
    ...
    let mut driver = match BootFbDriver::new() {
        Some(d) => {
            debug!("display_bootfb: Driver initialized successfully ({}x{})", d.fb.width, d.fb.height);
