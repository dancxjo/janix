...
pub fn register_schema(
    ...
    if let Some(existing) = schemas.get(&kind) {
        ...
        // Let's log details
        let d1 = crate::symbols::resolve(existing.description)
...
pub fn get_schema_props(kind: SymbolId) -> Option<Vec<(SymbolId, PropType)>> {
    SCHEMAS
        .lock()
        .as_ref()
        .expect("Schemas not initialized")
        .get(&kind)
        .map(|s| s.props.iter().map(|(k, v)| (*k, *v)).collect())
}
...
pub fn decode_schema(definition: &[u8]) -> Result<(SymbolId, Vec<(SymbolId, PropType)>, Vec<SymbolId>, Vec<SchemaLink>), &'static str> {
    ...
    Ok((description, props, indexed, Vec::new()))
}
