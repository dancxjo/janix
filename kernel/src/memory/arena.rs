...
impl Arena {
    pub fn new(base: u64, size: usize, flags: ArenaFlags, generation: u64, tag: &'static str) -> Self {
        Self {
            base,
            size,
            cursor: 0,
...
impl ArenaHeap {
    ...
    pub fn add_arena(&mut self, mut arena: Arena) -> ArenaId {
        crate::kinfo!("ArenaHeap: Adding arena '{}' base={:x} size={} flags={:?}", arena.tag, arena.base, arena.size, arena.flags);
        
        // Find free slot
        let idx = self.arenas.iter().position(|slot| slot.is_none())
    ...
    pub fn alloc_evictable(&mut self, layout: Layout) -> Result<(ArenaId, u64, u32, NonNull<u8>), ()> {
         if layout.size() > self.stats.largest_alloc_request {
            self.stats.largest_alloc_request = layout.size();
        }
    ...
    pub fn evict_oldest(&mut self) -> Option<Arena> {
        ...
        crate::kinfo!("ArenaHeap: Evicting arena '{}' id={:?} used={}", arena.tag, id_to_evict, arena.used());
        ...
        self.stats.total_evictable_bytes = self.stats.total_evictable_bytes.saturating_sub(arena.used());
...
pub fn alloc_evictable<R: crate::BootRuntime>(layout: Layout) -> Result<crate::memory::handle::EvictHandle, ()> {
...
pub fn heap_status() {
    ...
    crate::kinfo!("  Evictions: {} (freed {} bytes)", stats.eviction_count, stats.bytes_freed_by_eviction);
