...
unsafe fn pci_read_config(bus: u8, dev: u8, func: u8, offset: u8) -> u32 {
    let address = PCI_ENABLE_BIT | ((bus as u32) << 16) | ((dev as u32) << 11) | ((func as u32) << 8) | ((offset as u32) & 0xFC);
    ...
    FIntern: FnMut(&str) -> u64
{
    ...
    scan_bus(0, pci_bus_node, &mut create, &mut set, &mut link, &mut intern);
}
    ...
    FIntern: FnMut(&str) -> u64
{
    for dev in 0..32 {
        ...
        for func in 0..func_count {
            ...
            if (vendor_reg & 0xFFFF) == 0xFFFF { continue; }
    ...
    FIntern: FnMut(&str) -> u64
{
    ...
    // Simple hex loop
    fn nibble(n: u8) -> u8 { if n < 10 { n + b'0' } else { n - 10 + b'a' } }
    for i in 0..8 {
