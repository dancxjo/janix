...
fn handle_msg<R: BootRuntime>(
    ...
    let (status, value) = match msg.op {
        // Symbol operations
        RootOp::Intern { name, out_ptr } => root_handlers::handle_intern(interner, &name, out_ptr),

        // Graph core operations
        RootOp::GetKind { id, out_ptr } => root_handlers::handle_get_kind(graph, id, out_ptr),
        RootOp::CreateNode { kind, out_ptr } => root_handlers::handle_create_node(graph, journal, interner, kind, out_ptr),
        ...
        // Property operations
        RootOp::PropGet { id, key, out_ptr } => root_handlers::handle_prop_get(graph, interner, id, key, out_ptr),
        RootOp::PropSet { id, key, value } => root_handlers::handle_prop_set(graph, journal, interner, id, key, value),

        // Bytespace operations
        RootOp::BytespaceCreate { len, flags, format, out_ptr } => {
            root_handlers::handle_bytespace_create::<R>(graph, journal, interner, &msg, len, flags, format, out_ptr)
        }
        RootOp::BytespaceCreateFromPtr { ptr, len, out_ptr } => {
            root_handlers::handle_bytespace_create_from_ptr::<R>(graph, journal, interner, ptr, len, out_ptr)
        }
        ...
        RootOp::WatchSubscribe { target_id, mask, out_ptr } => {
            root_handlers::handle_watch_subscribe(graph, interner, target_id, mask, out_ptr)
        }
        RootOp::StreamPoll { stream_id, max: _, out_ptr: _ } => {
            root_handlers::handle_stream_poll(graph, &msg, stream_id)
        }
        RootOp::WatchOpen { mode, start_seq, query, filter, out_ptr } => {
            root_handlers::handle_watch_open(graph, interner, mode, start_seq, query, filter, out_ptr)
        }
        RootOp::WatchNext { id, .. } => {
