...
pub fn sys_stream_open(node_id: usize) -> SysResult<usize> {
    ...
    // Client handles
    let c_write = table.alloc(cs_port_id, HandleMode::Write).ok_or(Errno::ENOMEM)?;
    let c_read = table.alloc(sc_port_id, HandleMode::Read).ok_or(Errno::ENOMEM)?;
    
    // Server handles
    let s_read = table.alloc(cs_port_id, HandleMode::Read).ok_or(Errno::ENOMEM)?;
    let s_write = table.alloc(sc_port_id, HandleMode::Write).ok_or(Errno::ENOMEM)?;
    ...
    let l_port = {
        let table = GLOBAL_HANDLE_TABLE.lock();
        let entry = table.get(listener_handle, HandleMode::Write).ok_or(Errno::EBADF)?;
...
pub fn sys_stream_poll(handle: usize, _events: usize, _timeout: usize) -> SysResult<usize> {
    ...
    let table = GLOBAL_HANDLE_TABLE.lock();
    let entry = table.get(Handle(handle as u32), HandleMode::Read).ok_or(Errno::EBADF)?;
