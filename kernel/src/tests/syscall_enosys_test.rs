...
pub fn run_selftest() {
    ...
    if ret >= 0 {
        crate::kinfo!("ENOSYS GUARD TEST: FAIL - Old SYS_RTC_READ (9) should fail but returned {}", ret);
        return;
    }
    
    let ret = crate::syscall::dispatch::dispatch(OLD_SYS_PORT_SEND, [0; 6]);
    if ret >= 0 {
        crate::kinfo!("ENOSYS GUARD TEST: FAIL - Old SYS_PORT_SEND (33) should fail but returned {}", ret);
        return;
    }
    
    let ret = crate::syscall::dispatch::dispatch(OLD_SYS_ROOT_GET_KIND, [0; 6]);
    if ret >= 0 {
        crate::kinfo!("ENOSYS GUARD TEST: FAIL - Old SYS_ROOT_GET_KIND (256) should fail but returned {}", ret);
        return;
    }
