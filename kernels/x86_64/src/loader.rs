...
use kernel_core::fs::iso9660::{Iso9660Reader, BlockReader};
...
pub extern "C" fn scan_boot_fs_task(arg: u64) {
    ...
    let iso = match Iso9660Reader::new(boxed_reader) {
        Some(i) => Arc::new(i),
        None => {
             unsafe { Bridge.log("loader: Failed to init ISO reader\n"); }
             return;
        }
    };
...
pub fn process_file(
    ...
    if should_spawn {
        ...
        if let Some(img) = loaded {
             ...
             let end_page = Page::<Size4KiB>::containing_address(VirtAddr::new(heap_virt_start + heap_size - 1u64));
              for page in Page::range_inclusive(start_page, end_page) {
                 let frame = frame_allocator.allocate_frame().unwrap();
                  let flags = PageTableFlags::PRESENT | PageTableFlags::WRITABLE | PageTableFlags::USER_ACCESSIBLE;
                  unsafe { 
             ...
             let p_body = ProcessBody {
                 ...
                 name: k.symbols.intern(name).unwrap_or(models::builtins::symbols::SYM_PROCESS),
            ...
            let tb = ThingBody::from(&TypedBytes {
                    ...
                    .unwrap();
            let prog_id = k.graph.create_thing(models::builtins::ids::THING_BOOT_PROGRAM_KIND, tb);
