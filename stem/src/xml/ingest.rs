...
use crate::xml::{XmlReader, Event};
use crate::xml::model::{kinds, rels, props};
...
impl GraphApply for SysGraphApply {
    ...
    fn intern(&mut self, s: &str) -> Result<u64, XmlIngestError> {
        crate::thing::sys::intern(s).map_err(|_| XmlIngestError::GraphError).map(|v| v as u64)
    }
}
    ...
    loop {
        match reader.next() {
            Some(Ok(Event::StartElement { name, attributes })) => {
                ...
                // Attributes
                for (k, v) in attributes {  // Uses IntoIterator
                    nodes_created += 1;
            ...
            Some(Ok(Event::Text(text))) => { // Fixed variant
...
mod tests {
    ...
    use abi::ids::HandleId; // For from_u64
    ...
    fn test_ingest_basic_svg() {
        ...
        let res = ingest_xml_to_graph(xml.as_bytes(), XmlIngestOptions::default(), &mut graph).expect("ingest failed");
        ...
        // Doc -> Root
        assert!(graph.edges.iter().any(|(src, rel, dst)| 
            src.to_u64_lossy() == 1 && rel == "HAS_ROOT" && dst.to_u64_lossy() == 2
        ));
        
        // Root is <svg>
        // Check props? We need to look up in interner.
        let svg_sym = graph.interner.get("svg").expect("svg interned");
        assert_eq!(graph.props.get(&(res.root_element, "tag".to_string())).expect("has tag"), &format!("{}", svg_sym));
    ...
    fn test_limit_depth() {
        ...
        let _err = ingest_xml_to_graph(xml.as_bytes(), opts, &mut graph).err().expect("should fail");
    }
}
