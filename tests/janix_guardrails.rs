...
fn guardrail_scheduler_first_spawn_exec_exist() {
    ...
    assert_eq!(SYS_SPAWN_THREAD, 0x1004, "SYS_SPAWN_THREAD ABI number changed");
    assert_eq!(SYS_SPAWN_PROCESS, 0x1005, "SYS_SPAWN_PROCESS ABI number changed");
    assert_eq!(SYS_SPAWN_PROCESS_EX, 0x1006, "SYS_SPAWN_PROCESS_EX ABI number changed");
...
fn guardrail_userland_drivers_device_primitives_exist() {
    assert_eq!(SYS_DEVICE_CLAIM, 0x5000, "SYS_DEVICE_CLAIM ABI number changed");
    assert_eq!(SYS_DEVICE_MAP_MMIO, 0x5002, "SYS_DEVICE_MAP_MMIO ABI number changed");
    assert_eq!(SYS_DEVICE_IRQ_SUBSCRIBE, 0x5007, "SYS_DEVICE_IRQ_SUBSCRIBE ABI number changed");
    assert_eq!(SYS_DEVICE_IRQ_WAIT, 0x5008, "SYS_DEVICE_IRQ_WAIT ABI number changed");
...
fn guardrail_vfs_first_fs_syscalls_exist() {
    ...
    for &n in &[SYS_FS_OPEN, SYS_FS_CLOSE, SYS_FS_READ, SYS_FS_WRITE, SYS_FS_STAT, SYS_FS_MOUNT] {
        assert!(
            n >= 0x4000,
...
fn guardrail_no_fork_spawn_exec_numbers_are_distinct() {
    ...
    assert_ne!(SYS_SPAWN_PROCESS, SYS_TASK_EXEC, "spawn and exec must be distinct");
    assert_ne!(SYS_SPAWN_PROCESS_EX, SYS_TASK_EXEC, "spawn_ex and exec must be distinct");
