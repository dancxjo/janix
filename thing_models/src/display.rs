...
impl crate::Thing for DisplayPresentRequest {
    ...
    fn schema() -> &'static [(&'static str, crate::PropType)] {
        &[
            ("framebuffer_id", crate::PropType::U64),
            ("frame_index", crate::PropType::U64),
            ("requested_at_ns", crate::PropType::U64),
            ("presented_at_ns", crate::PropType::U64),
            ("completed", crate::PropType::Bool),
            ("damage_count", crate::PropType::U64),
            ("damage_rects", crate::PropType::Blob),
        ]
    }

    fn to_props(&self, out: &mut Vec<(crate::PropKey, crate::PropValue)>) {
        out.push(("framebuffer_id".to_string(), crate::PropValue::U64(self.framebuffer_id.0)));
        out.push(("frame_index".to_string(), crate::PropValue::U64(self.frame_index)));
        out.push(("requested_at_ns".to_string(), crate::PropValue::U64(self.requested_at_ns)));
        if let Some(val) = self.presented_at_ns {
            out.push(("presented_at_ns".to_string(), crate::PropValue::U64(val)));
        }
        out.push(("completed".to_string(), crate::PropValue::Bool(self.completed)));
        out.push(("damage_count".to_string(), crate::PropValue::U64(self.damage_count as u64)));
        out.push(("damage_rects".to_string(), crate::PropValue::Blob(self.damage_rects.clone())));
    }

    fn from_props(id: ThingId, props: &[Option<(crate::PropKey, crate::PropValue)>]) -> Self {
        ...
        for prop_opt in props {
            if let Some((k, v)) = prop_opt {
                match (k.as_str(), v) {
                    ("framebuffer_id", crate::PropValue::U64(val)) => framebuffer_id = ThingId(*val),
                    ("frame_index", crate::PropValue::U64(val)) => frame_index = *val,
                    ("requested_at_ns", crate::PropValue::U64(val)) => requested_at_ns = *val,
                    ("presented_at_ns", crate::PropValue::U64(val)) => presented_at_ns = Some(*val),
                    ("completed", crate::PropValue::Bool(val)) => completed = *val,
                    ("damage_count", crate::PropValue::U64(val)) => damage_count = *val as u32,
                    ("damage_rects", crate::PropValue::Blob(val)) => damage_rects = val.clone(),
                    _ => {}
                }
            }
        }
