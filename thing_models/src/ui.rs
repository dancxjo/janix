...
impl Thing for Mode {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        out.push((
            graph_kinds::PROP_MODE_INDEX.to_string(),
        ...
        if let Some(place) = self.place_id {
            out.push((
                graph_kinds::PROP_MODE_PLACE.to_string(),
                PropValue::U64(place.0),
            ));
        }
        out.push((
            graph_kinds::PROP_MODE_ACTIVE.to_string(),
            PropValue::Bool(self.active),
        ));
        if let Some(policy) = self.layout_policy {
            out.push((
                graph_kinds::PROP_MODE_LAYOUT_POLICY.to_string(),
                PropValue::I64(policy),
            ));
        }
    }
...
impl Thing for Place {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        ...
        if let Some(mode) = &self.layout_mode {
            out.push((
                graph_kinds::PROP_LAYOUT_MODE.to_string(),
                PropValue::Str(mode.clone()),
            ));
        }
    }
...
impl Thing for Window {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        out.push((
            graph_kinds::PROP_PLACE_ID.to_string(),
            PropValue::U64(self.place_id.0),
        ));
        out.push((
            graph_kinds::PROP_WINDOW_X.to_string(),
            PropValue::I64(self.x as i64),
        ));
        out.push((
            graph_kinds::PROP_WINDOW_Y.to_string(),
            PropValue::I64(self.y as i64),
        ));
        out.push((
        ...
        out.push((
            graph_kinds::PROP_TITLE.to_string(),
            PropValue::Str(self.title.clone()),
        ));
        out.push((
            graph_kinds::PROP_DRAGGABLE.to_string(),
            PropValue::Bool(self.draggable),
        ));
        out.push((
            graph_kinds::PROP_RESIZABLE.to_string(),
            PropValue::Bool(self.resizable),
        ));
        out.push((
            graph_kinds::PROP_CLOSABLE.to_string(),
            PropValue::Bool(self.closable),
        ));
        out.push((
            graph_kinds::PROP_MINIMIZABLE.to_string(),
            PropValue::Bool(self.minimizable),
        ));
    }
...
impl Thing for Surface {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        out.push((
            graph_kinds::PROP_WINDOW_ID.to_string(),
            PropValue::U64(self.window_id.0),
        ));
        out.push((
            graph_kinds::PROP_SURFACE_KIND.to_string(),
        ...
        out.push((
            ...
            PropValue::Str(self.format.clone()),
        ));
        if let Some(sb) = self.shared_buffer_id {
            out.push((
                graph_kinds::PROP_SHARED_BUFFER_ID.to_string(),
                PropValue::U64(sb.0),
            ));
        }
...
impl Thing for ModeSwitchEvent {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        ...
        out.push((
            graph_kinds::PROP_TIMESTAMP.to_string(),
            PropValue::U64(self.timestamp),
        ));
    }
...
impl Thing for View {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        ...
        out.push((
            graph_kinds::PROP_WINDOW_X.to_string(),
            PropValue::I64(self.x),
        ));
        out.push((
            graph_kinds::PROP_WINDOW_Y.to_string(),
            PropValue::I64(self.y),
        ));
        out.push((
            graph_kinds::PROP_WIDTH.to_string(),
            PropValue::U64(self.width as u64),
        ));
        out.push((
            graph_kinds::PROP_HEIGHT.to_string(),
            PropValue::U64(self.height as u64),
        ));
        out.push((
            graph_kinds::PROP_VISIBLE.to_string(),
            PropValue::Bool(self.visible),
        ));
        if let Some(text) = &self.text {
            out.push((
                graph_kinds::PROP_TEXT.to_string(),
        ...
        if let Some(font) = &self.font {
            out.push((
                ...
                PropValue::Str(font.clone()),
            ));
        }
        out.push((
            graph_kinds::PROP_FONT_SIZE.to_string(),
            PropValue::U64(self.font_size),
        ));
        out.push((
            graph_kinds::PROP_FG_COLOR.to_string(),
            PropValue::U64(self.fg_color as u64),
        ));
        out.push((
            graph_kinds::PROP_BG_COLOR.to_string(),
            PropValue::U64(self.bg_color as u64),
        ));
    }
    ...
    fn from_props(id: ThingId, props: &[Option<(PropKey, PropValue)>]) -> Self {
        ...
        for prop in props.iter().flatten() {
            match prop.0.as_str() {
                ...
                graph_kinds::PROP_VISIBLE => {
                    if let PropValue::Bool(v) = prop.1 {
                        visible = v;
                    }
                }
                graph_kinds::PROP_TEXT => {
                ...
                graph_kinds::PROP_FONT_SIZE => {
                    if let PropValue::U64(v) = prop.1 {
                        font_size = v;
                    }
                }
                graph_kinds::PROP_FG_COLOR => {
                    if let PropValue::U64(v) = prop.1 {
                        fg_color = v as u32;
                    }
                }
                graph_kinds::PROP_BG_COLOR => {
                    if let PropValue::U64(v) = prop.1 {
                        bg_color = v as u32;
                    }
                }
...
impl Thing for Cursor {
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        ...
        out.push((
            graph_kinds::PROP_CURSOR_SHAPE.to_string(),
            PropValue::Str(self.shape.clone()),
        ));
        out.push((
            graph_kinds::PROP_VISIBLE.to_string(),
            PropValue::Bool(self.visible),
