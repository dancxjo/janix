...
fn test_alarm_request_create_pending() {
    ...
    let props = AlarmRequest::create_pending(
        123456789,
        500,
        owner_process,
        owner_thread
    ...
    let props_map: std::collections::HashMap<String, PropValue> = props
        .into_iter()
        .map(|(k, v)| (k, v))
        .collect();

    assert_eq!(props_map.get("target_unix_seconds"), Some(&PropValue::I64(123456789)));
    assert_eq!(props_map.get("target_unix_nanos"), Some(&PropValue::U64(500)));
