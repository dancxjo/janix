...
use abi::ThingId;
pub use abi::resident::{
    ResidentAllocResp, ResidentError, ResidentErrorCode, ResidentMapPerms, ResidentMapResp,
    RestPolicy, RestResp,
};
...
impl<T> Resident<T> {
        ...
        F: FnMut(&ResidentHeader, &[u8]) -> R,
    {
        unsafe {
            ...
            loop {
                ...
                let res = f(
                    &*header_ptr,
                    core::slice::from_raw_parts(self.ptr, self.byte_len),
                );
    ...
    where
        F: FnOnce(&mut ResidentHeader, &mut [u8]) -> R,
    {
        unsafe {
            ...
            let res = f(
                &mut *header_ptr,
                core::slice::from_raw_parts_mut(self.ptr, self.byte_len),
            );
