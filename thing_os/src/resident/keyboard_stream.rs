use super::Resident;
use crate::prelude::*;
pub use abi::keyboard_stream::{KeyboardEntry, KeyboardStreamHeader};
use alloc::vec::Vec;
    ...
    pub fn init(&mut self, capacity: u64) {
        self.obj.with_write(|header, data| {
            unsafe {
                ...
                // Ensure space
                let needed = data_off
                    + core::mem::size_of::<KeyboardStreamHeader>()
                    + (capacity as usize * core::mem::size_of::<KeyboardEntry>());
                if needed > data.len() {
                    let stream_header_ptr =
                        data.as_mut_ptr().add(data_off) as *mut KeyboardStreamHeader;
                    let mut header_val = stream_header_ptr.read_unaligned();
