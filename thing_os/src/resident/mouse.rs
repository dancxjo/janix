use super::Resident;
pub use abi::mouse_stream::{MouseEntry, MouseStreamHeader};
use alloc::vec::Vec;
...
impl<T> MouseStreamMapped<T> {
    ...
    pub fn append(&mut self, entry: MouseEntry) {
        self.obj.with_write(|header, data| unsafe {
            let data_off = header.data_off as usize;
            if data_off >= data.len() {
                return;
            }
    ...
    pub fn init(&mut self, capacity: u32) {
        self.obj.with_write(|header, data| {
            unsafe {
                ...
                // Ensure space
                let needed = data_off
                    + core::mem::size_of::<MouseStreamHeader>()
                    + (capacity as usize * core::mem::size_of::<MouseEntry>());
                if needed > data.len() {
                    // Cannot init, too small.
                    // Adjust capacity? Or just fail silently (v1)?
                    // Set capacity to 0.
                    let stream_header_ptr =
                        data.as_mut_ptr().add(data_off) as *mut MouseStreamHeader;
