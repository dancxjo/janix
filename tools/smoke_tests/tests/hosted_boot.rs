...
fn hosted_boot() {
    ...
    assert!(stdout.contains("THINGOS: kernel entry"), "Missing kernel entry log");
    assert!(stdout.contains("THINGOS: bridge online"), "Missing bridge online log");
    assert!(stdout.contains("THINGOS: idle loop"), "Missing idle loop log");
