...
impl Compositor {
    ...
    fn apply_mouse_event(&mut self, dx: i64, dy: i64, buttons: u64, layout: &[StackedWindow]) {
        ...
        let previous = self.cursor.apply_packet(
            ...
            dy,
            buttons,
            self.fb_info.width as i32,
            self.fb_info.height as i32,
        );
    ...
    fn continue_drag(&mut self) {
        ...
        let max_x = (self.fb_info.width as i32 - drag.width).max(0);
        let max_y = (self.fb_info.height as i32 - drag.height).max(0);
