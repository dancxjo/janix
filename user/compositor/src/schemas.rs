...
impl Thing for Surface {
    const KIND: &'static str = "pkg.compositor.Surface";
    const DESCRIPTION: &'static str = "A compositor surface";

    fn schema() -> &'static [(&'static str, PropType)] {
        &[
            ("width", PropType::U64),
            ("height", PropType::U64),
            ("format", PropType::Symbol),
    ...
    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        ...
        out.push(("format".to_string(), PropValue::Symbol(self.format)));
        ...
        for prop_opt in props {
            if let Some((k, v)) = prop_opt {
                match (k.as_str(), v) {
                    ...
                    ("format", PropValue::Symbol(val)) => format = *val,
...
pub struct RenderTree {
    pub id: ThingId,
    pub root_surface: ThingId,
}

impl Thing for RenderTree {
    const KIND: &'static str = "pkg.compositor.RenderTree";
    const DESCRIPTION: &'static str = "The render tree root";

    fn schema() -> &'static [(&'static str, PropType)] {
        &[
            ("root_surface", PropType::U64),
        ]
    }

    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        out.push(("root_surface".to_string(), PropValue::U64(self.root_surface.0)));
    }

    fn from_props(id: ThingId, props: &[Option<(PropKey, PropValue)>]) -> Self {
        let mut root_surface = ThingId(0);

        for prop_opt in props {
            if let Some((k, v)) = prop_opt {
                match (k.as_str(), v) {
                    ("root_surface", PropValue::U64(val)) => root_surface = ThingId(*val),
                    _ => {}
                }
            }
        }
...
impl Thing for FrameRenderIntent {
    const KIND: &'static str = "pkg.compositor.FrameRenderIntent";
    const DESCRIPTION: &'static str = "Draw the current scene";

    fn schema() -> &'static [(&'static str, PropType)] {
        &[
            ("target_buffer", PropType::U64),
            ("state", PropType::U64),
        ]
    }

    fn to_props(&self, out: &mut Vec<(PropKey, PropValue)>) {
        out.push(("target_buffer".to_string(), PropValue::U64(self.target_buffer as u64)));
        out.push(("state".to_string(), PropValue::U64(self.state as u32 as u64)));
    }

    fn from_props(id: ThingId, props: &[Option<(PropKey, PropValue)>]) -> Self {
        let mut target_buffer = 0;
        let mut state = IntentState::Pending;

        for prop_opt in props {
            if let Some((k, v)) = prop_opt {
                match (k.as_str(), v) {
                    ("target_buffer", PropValue::U64(val)) => target_buffer = *val as u32,
