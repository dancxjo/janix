...
fn test_manual_and_derived_thing_compatibility() {
    ...
    assert_eq!(manual_props.len(), 2, "ManualCounter should have 2 properties");
...
fn test_cross_compatible_serialization() {
    ...
    let props_opts: Vec<Option<(PropKey, PropValue)>> = manual_props.into_iter().map(Some).collect();
