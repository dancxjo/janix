...
fn register_disk(port: &mut AhciPort) {
    ...
    info!(
        ...
        disk_id.to_u64_lossy(), port.port_num, port.sector_count, port.supports_lba48, model_str, write_handle
    );
}
...
fn register_atapi_disk(port: &mut AhciPort) {
    ...
    info!(
        ...
        node_id.to_u64_lossy(), port.port_num, model_str, write_handle
    );
}
...
fn main(_arg: usize) -> ! {
    ...
    let handles: Vec<PortHandle> = ports.iter()
        .filter_map(|p| p.read_port_handle)
        .collect();
...
fn handle_block_device_request(port: &AhciPort, request_data: &[u8], port_handle: PortHandle) {
    ...
    match request_type {
        0 => handle_identify(port, port_handle),
        1 => handle_read(port, &request_data[1..], port_handle),
