...
fn stop_port(hba_base: u64, port: u32) {
    ...
    for _ in 0..1000 {
        if mmio_read32(pb, PORT_CMD) & PORT_CMD_CR == 0 {
            break;
        }
    }
    ...
    mmio_write32(pb, PORT_CMD, cmd);
    for _ in 0..1000 {
        if mmio_read32(pb, PORT_CMD) & PORT_CMD_FR == 0 {
            break;
        }
    }
}
...
fn main(_arg: usize) -> ! {
    ...
    let claim_handle = match stem::syscall::device_claim(pci_id.0) {
        Ok(h) => {
            info!("AHCI: Claimed PCI device {} handle={}", pci_id.0, h);
            h
        }
        Err(e) => {
    ...
    let dma_virt = match stem::syscall::device_alloc_dma(claim_handle, dma_pages) {
        Ok(addr) => {
            info!("AHCI: DMA virt=0x{:x}", addr);
            addr
        }
        Err(e) => {
            error!("AHCI: DMA alloc failed: {:?}", e);
            loop {
                stem::sleep(Duration::from_secs(60));
            }
        }
    ...
    for port_num in 0..32u32 {
        ...
        match sig {
            ...
            SATA_SIG_ATAPI => {
                info!("AHCI: Port {} - SATAPI (skip)", port_num);
                continue;
            }
            SATA_SIG_SEMB => {
