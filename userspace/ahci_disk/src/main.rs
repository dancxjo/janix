...
fn publish_content_file_indexed(
    ...
    match thingsys::create_node(kinds::CONTENT_FILE) {
        ...
        Err(_) => {
            warn!("AHCI: Failed to create File node for '{}'", name);
            None
        }
    }
}
            ...
            if in_hot_set {
                    ...
                    Ok(data) => {
                        match publish_iso_file(
                            ...
                            *index,
                            ...
                            Ok(_) => {
                                info!("AHCI: Published (hot-set) '{}' ({} bytes)", path_with_slash, entry.size);
                                published += 1;
                                *index += 1;
                            }
                ...
                match publish_iso_file(
                    ...
                    *index,
                    ...
                    Err(e) => {
                        warn!("AHCI: Failed to publish metadata for '{}': {}", full_path, e);
                    }
                }
            }
        }
