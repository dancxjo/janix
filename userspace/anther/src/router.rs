...
pub fn match_route<'a>(method: Method, path: &'a str) -> Option<ApiRoute<'a>> {
    ...
    let segments: alloc::vec::Vec<&str> = path_part.split('/').filter(|s| !s.is_empty()).collect();
...
mod tests {
    ...
    fn test_discovery_route() {
        assert_eq!(
            match_route(Method::Get, "/api/v1/"),
            Some(ApiRoute::Discovery)
        );
        assert_eq!(
            match_route(Method::Get, "/api/v1"),
            Some(ApiRoute::Discovery)
    ...
    fn test_bytespace_routes() {
        ...
        assert_eq!(
            ...
            Some(ApiRoute::GetBytespaceMetadata {
                thing_id: "1",
                key: "content"
            })
        );
