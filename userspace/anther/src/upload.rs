...
use stem::thing::sys::{bytespace_create, bytespace_map, bytespace_unmap, create_node, prop_set, intern};
...
pub fn handle_upload(req: &Request<'_>, body: &[u8]) -> Vec<u8> {
    ...
    info!("anther: Receiving upload '{}' ({} bytes)", basename, body.len());
    ...
    let mime = if basename.ends_with(".png") { "image/png" }
        else if basename.ends_with(".jpg") { "image/jpeg" }
        else if basename.ends_with(".svg") { "image/svg+xml" }
        else if basename.ends_with(".ttf") { "font/ttf" }
        else if basename.ends_with(".txt") { "text/plain" }
