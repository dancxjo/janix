...
pub fn blit_rgba8888_over_scalar_row(dst: &mut [u32], src: &[u32]) {
    let len = dst.len().min(src.len());
    let dst = &mut dst[0..len];
    let src = &src[0..len];

    crate::trace_counter!("raster.blit.rgba.px_total", len as u64);
    ...
    for (d, &s) in dst.iter_mut().zip(src.iter()) {
        ...
        if sa == 255 {
            *d = s;
            crate::trace_counter!("raster.blit.rgba.opaque_px_total", 1);
        } else {
             let sr = (s >> 16) & 0xFF;
             let sg = (s >> 8) & 0xFF;
             let sb = s & 0xFF;
             ...
             *d = (out_a << 24) | (out_r << 16) | (out_g << 8) | out_b;
             crate::trace_counter!("raster.blit.rgba.blend_px_total", 1);
        }
    }
}
...
pub fn blit_a8_tinted_over_scalar_row(dst: &mut [u32], src_a8: &[u8], color: u32, tint_a: u8) {
    ...
    crate::trace_counter!("raster.blit.a8.px_total", len as u64);
    ...
    while i < len {
        ...
        if sa == 0 {
            i += 1;
            crate::trace_counter!("raster.blit.a8.zero_px_skipped_total", 1);
            continue;
        }
        ...
        if ea == 255 {
            dst[i] = (255 << 24) | (tr << 16) | (tg << 8) | tb;
             ...
             let dr = (dv >> 16) & 0xFF;
             let dg = (dv >> 8) & 0xFF;
             let db = dv & 0xFF; // Fixed masking here for clarity, though & 0xFF is implied
