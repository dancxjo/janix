#[cfg(test)]
mod tests {
    use crate::blit::{blit_rgba8888_over, blit_a8_tinted_over};
    ...
    fn test_a8_tinted_all_255_mask_opaque() {
        ...
        // Should all be red
        for &pixel in &dst {
            assert_eq!(pixel, color, "All-255 mask with opaque color should fill with color");
        }
    }
    ...
    fn test_a8_tinted_vs_scalar() {
        ...
        stem::simd::scalar::composite_solid_masked_over_scalar(
            &mut dst_scalar, len, &mask, len, len, 1, color
        );
    ...
    fn test_a8_tinted_with_modulation() {
        ...
        let mut dst_scalar = dst_orig.clone();
        stem::simd::scalar::composite_solid_masked_over_scalar(
            &mut dst_scalar, len, &mask, len, len, 1, effective_color
        );
