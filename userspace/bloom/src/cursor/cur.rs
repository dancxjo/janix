...
pub fn load_cur(data: &[u8]) -> Option<CursorFrame> {
    ...
    Some(CursorFrame::new(
        pixels, width, height, hotspot_x, hotspot_y,
    ))
}
...
fn decode_dib(data: &[u8], width: u32, height: u32) -> Option<Vec<u32>> {
    ...
    let use_height = if actual_height > 0 {
        actual_height
    } else {
        height
    };
    ...
    match bpp {
            ...
            for y in 0..use_height {
                ...
                for x in 0..use_width {
                    ...
                    if is_transparent {
                        ...
                        let r = pixel_data[px_offset + 2];
                        pixels
                            .push(0xFF000000 | ((r as u32) << 16) | ((g as u32) << 8) | (b as u32));
                    } else {
                        pixels.push(0);
                    }
                }
        ...
        _ => {
            for _ in 0..(use_width * use_height) {
                pixels.push(0);
            }
        }
    }
...
fn premultiply(r: u8, g: u8, b: u8, a: u8) -> (u8, u8, u8) {
    if a == 255 {
        ...
        let a32 = a as u32;
        (
            ((r as u32 * a32 + 127) / 255) as u8,
            ((g as u32 * a32 + 127) / 255) as u8,
            ((b as u32 * a32 + 127) / 255) as u8,
        )
    }
}
