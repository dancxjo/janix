...
impl VirglCommandBuilder {
    ...
    pub fn cmd_clear(&mut self, buffers: u32, r: f32, g: f32, b: f32, a: f32, depth: f64, stencil: u32) {
    ...
    pub fn cmd_create_surface(&mut self, handle: u32, res_handle: u32, format: u32, first_element: u32, last_element: u32) {
    ...
    pub fn cmd_create_sampler_view(&mut self, handle: u32, res_handle: u32, format: u32, first_element: u32, last_element: u32, swizzle: u32) {
        self.cmds.push(cmd_header(virgl_cmd::CREATE_OBJECT, virgl_object::SAMPLER_VIEW, 6));
        self.cmds.push(handle);
        self.cmds.push(res_handle);
        self.cmds.push(format);
        self.cmds.push(first_element);
    ...
    pub fn cmd_destroy_object(&mut self, handle: u32, object_type: u32) {
        self.cmds.push(cmd_header(virgl_cmd::DESTROY_OBJECT, object_type, 1));
        self.cmds.push(handle);
    }

    /// Set viewport state  
    pub fn cmd_set_viewport(&mut self, start_slot: u32, scale: [f32; 3], translate: [f32; 3]) {
        self.cmds.push(cmd_header(virgl_cmd::SET_VIEWPORT_STATE, 0, 7));
    ...
    pub fn cmd_set_scissor(&mut self, start_slot: u32, minx: u16, miny: u16, maxx: u16, maxy: u16) {
        self.cmds.push(cmd_header(virgl_cmd::SET_SCISSOR_STATE, 0, 3));
        self.cmds.push(start_slot);
...
impl GpuCompositor {
    ...
    pub fn set_scanout_resource(&mut self, resource_id: u32, width: u32, height: u32) {
        ...
        self.width = width;
        self.height = height;
        info!("GpuCompositor: scanout resource {} ({}x{})", resource_id, width, height);
    }
    ...
    pub fn build_setup_commands(&mut self) -> &[u8] {
        ...
        self.cmd_builder.cmd_set_framebuffer(1, 0, &[self.scanout_surface]);
    ...
    pub fn build_clear_commands(&mut self, r: f32, g: f32, b: f32, a: f32) -> &[u8] {
        self.cmd_builder.clear();
        self.cmd_builder.cmd_clear(clear_bits::COLOR, r, g, b, a, 1.0, 0);
        self.cmd_builder.as_bytes()
    }
    ...
    pub fn register_texture(&mut self, window_id: u64, resource_id: u32, width: u32, height: u32, generation: u64) {
        let surface_handle = self.alloc_texture_handle();
        self.textures.insert(window_id, GpuTexture {
            resource_id,
            width,
            height,
            generation,
    ...
    pub fn render_quads(&mut self, quads: &[Quad]) -> &[u8] {
        ...
        for quad in sorted_quads {
            ...
            if let Some(tex) = self.textures.values().find(|t| t.resource_id == quad.texture_id) {
                let src_rect = quad.src_rect.unwrap_or(Rect::new(0, 0, tex.width, tex.height));
    ...
    pub fn build_blit_texture(&mut self, tex_resource: u32, tex_width: u32, tex_height: u32, dst_x: i32, dst_y: i32) -> &[u8] {
        self.cmd_builder.clear();
