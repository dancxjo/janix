...
use abi::display::{
    DISPLAY_OP_GET_INFO, DISPLAY_OP_IMPORT_BUFFER, DISPLAY_OP_COMMIT,
    DisplayInfo, BufferHandle, CommitRequest, PlaneCommit, BufferId, PlaneId, CommitFlags
};
...
fn main(_arg: usize) -> ! {
    ...
    let info_slice = unsafe { core::slice::from_raw_parts_mut(&mut info_payload as *mut _ as *mut u8, core::mem::size_of::<DisplayInfo>()) };
    ...
    let buffer_id = match vfs_device_call(fd, DeviceKind::Display, DISPLAY_OP_IMPORT_BUFFER, &import_req as *const _ as u64) {
    ...
    let commit = PlaneCommit {
         plane_id: PlaneId(0),
         buffer_id,
         dest_rect: Rect { x: 0, y: 0, w: info_payload.preferred_mode.width, h: info_payload.preferred_mode.height },
         src_rect: Rect { x: 0, y: 0, w: wallpaper.width, h: wallpaper.height },
         z_order: 0,
         alpha: 255,
         _reserved: [0; 7],
    };
    ...
    match vfs_device_call(fd, DeviceKind::Display, DISPLAY_OP_COMMIT, &commit_req as *const _ as u64) {
        Ok(_) => {
