...
/// Emit an additional compact sample after this many frames (~15s at 60 FPS)
pub const PERF_LATE_SAMPLE_FRAME: u64 = 900;
...
pub fn end_frame() -> Option<PerfFrame> {
    ...
    if (frame_no % PERF_PRINT_CADENCE == 0 && frame_no > 0)
        || (frame_no > 0 && frame_no % PERF_LATE_SAMPLE_FRAME == 0)
    {
    ...
    if state.history.len() >= 120 {
        ...
        for (name, values) in agg_spans {
            ...
            crate::log!(
                "  {:<25} avg={:>6.2}ms p50={:>6.2}ms p95={:>6.2}ms max={:>4}ms",
                name,
                avg,
                p50,
                p95,
...
fn print_compact_stats(frame: &PerfFrame, frame_no: u64) {
    ...
    let t_layout_ms = frame
        .counters
        .get("text.layout_ns")
        .map(|&ns| ns as f64 / 1_000_000.0)
        .unwrap_or(0.0);
    let t_raster_ms = frame
        .counters
        .get("text.raster_ns")
        .map(|&ns| ns as f64 / 1_000_000.0)
        .unwrap_or(0.0);
    ...
    let blit_rgba_calls = frame
        .counters
        .get("raster.blit.rgba.count")
        .copied()
        .unwrap_or(0);
    let blit_rgba_pixels = frame
        .counters
        .get("raster.blit.rgba.pixels")
        .copied()
        .unwrap_or(0);
    let blit_masked_calls = frame
        .counters
        .get("raster.blit.a8_masked.count")
        .copied()
        .unwrap_or(0);
    let blit_masked_pixels = frame
        .counters
        .get("raster.blit.a8_masked.pixels")
        .copied()
        .unwrap_or(0);
    ...
    crate::log!(
        "[PERF]   ops: fill={} blit={} text={} | text: {:.2}ms (L={:.2} R={:.2} B={:.2}) glyphs={}",
        ops_fill,
        ops_blit,
        ops_text,
        text_ns as f64 / 1_000_000.0,
        t_layout_ms,
        t_raster_ms,
        t_blit_ms,
        text_glyphs
