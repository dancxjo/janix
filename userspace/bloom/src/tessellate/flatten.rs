...
fn flatten_quad_recursive(
    ...
    let chord_mid = VirPoint::new((p0.x + p1.x) * 0.5, (p0.y + p1.y) * 0.5);
    ...
    if dist_sq <= tolerance * tolerance {
        ...
        let cp0 = VirPoint::new((p0.x + cp.x) * 0.5, (p0.y + cp.y) * 0.5);
        let cp1 = VirPoint::new((cp.x + p1.x) * 0.5, (cp.y + p1.y) * 0.5);
...
fn lerp_point(p0: VirPoint, p1: VirPoint, t: f32) -> VirPoint {
    VirPoint::new(p0.x + (p1.x - p0.x) * t, p0.y + (p1.y - p0.y) * t)
}
