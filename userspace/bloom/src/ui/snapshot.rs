...
use alloc::vec;
use alloc::vec::Vec;
use stem::thing::sys::{get_kind, prop_get};
...
impl UiSnapshot {
    ...
    fn traverse(&mut self, id: ThingId, kind_ids: &KindIds, keys: &UiKeys) {
        ...
        let common_keys = [
            (keys.x, keys::UI_X),
            (keys.y, keys::UI_Y),
            (keys.w, keys::UI_WIDTH),
            (keys.h, keys::UI_HEIGHT),
            (keys.z_index, keys::UI_Z_INDEX),
            (keys.hidden, keys::UI_HIDDEN),
        ];
        ...
        let kind_keys = match kind {
            UiNodeKind::Text => vec![
                (keys.text, keys::UI_TEXT),
                (keys.font, keys::UI_FONT),
                (keys.font_size, keys::UI_FONT_SIZE),
                (keys.color, keys::UI_COLOR),
                (keys.center_x, keys::UI_CENTER_X),
                (keys.center_y, keys::UI_CENTER_Y),
            ],
            UiNodeKind::Panel | UiNodeKind::Window => vec![
                (keys.radius, keys::UI_RADIUS),
                (keys.title, keys::UI_TITLE),
                (keys.bg_color, keys::UI_BG_COLOR),
            ...
            if let Ok(val) = prop_get(id, key_str) {
                ...
                if key_id == keys.text || key_id == keys.font || key_id == keys.title {
                    if val != 0 {
                        ...
                        if let Ok(len) =
                            stem::thing::sys::bytespace_read(ThingId::from_u64(val), 0, &mut s_buf)
                        {
                            let s = String::from(
                                core::str::from_utf8(&s_buf[..len]).unwrap_or_default(),
                            );
