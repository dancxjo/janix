    ...
    fn resolve_collisions(&mut self) {
        ...
        for (_, indices) in &mut layers {
            ...
            if let (Some(&first), Some(&last)) = (indices.first(), indices.last()) {
                let total_w =
                    self.nodes[last].rect.x + self.nodes[last].rect.w - self.nodes[first].rect.x;
...
mod tests {
    ...
    fn test_graph_layout_no_overlap() {
        ...
        assert!(
            r_kernel.y <= r_host.y,
            "Kernel Y {} should be <= Host Y {}",
            r_kernel.y,
            r_host.y
        );
        assert!(
            r_host.y < r_root.y,
            "Host Y {} should be < Root Y {}",
            r_host.y,
            r_root.y
        );
        assert!(
            r_root.y < r_scheduler.y,
            "Root Y {} should be < Scheduler Y {}",
            r_root.y,
            r_scheduler.y
        );
    }
