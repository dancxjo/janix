...
fn main() -> Result<()> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            "--record" => { trace_record_path = iter.next(); },
            "--replay" => { trace_replay_path = iter.next(); },
            other => {
    ...
    let wasm_path = match wasm_path {
        Some(p) => p,
        None => {
            eprintln!("Usage: driver_wasm_host <wasm_file> [handle] [--record <file>] [--replay <file>]");
            return Ok(());
        }
    };

    let device_handle = handle_arg
