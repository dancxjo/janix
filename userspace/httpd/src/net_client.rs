...
impl NetClient {
    ...
    pub fn connect() -> Option<Self> {
        ...
        if count == 0 {
            warn!("httpd: Network stack not found");
            return None;
        }
        ...
        info!("httpd: Connected to netd socket API (netd_port={}, our_write={}, our_read={})", 
    ...
    pub fn tcp_listen(&self, port: u16) -> Option<u32> {
        ...
        if let Err(e) = port_send(self.netd_write_port, &msg) {
            warn!("httpd: Failed to send TCP_LISTEN: {:?}", e);
            return None;
        }
            ...
            match port_recv(self.our_read_port, &mut resp_buf) {
                Ok(len) if len >= 6 => {
                    ...
                    if resp_type == RESP_HANDLE {
                        ...
                        warn!("httpd: TCP_LISTEN returned error");
                        return None;
                    }
                }
                _ => {
    ...
    pub fn tcp_accept(&self, listen_handle: u32) -> Option<AcceptResult> {
        ...
        if let Err(e) = port_send(self.netd_write_port, &msg) {
            warn!("httpd: Failed to send TCP_ACCEPT: {:?}", e);
            return None;
        }
