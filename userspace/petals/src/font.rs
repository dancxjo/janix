...
use stem::syscall::vfs::{vfs_open, vfs_read, vfs_close, vfs_stat};
...
impl TextRenderer {
    ...
    pub fn draw_text(&self, canvas: &mut Canvas, text: &str, x: i32, y: i32, px_size: f32, color: u32) {
        ...
        for c in text.chars() {
            ...
            for gy in 0..metrics.height {
                ...
                if dy < 0 || dy >= canvas.height as i32 { continue; }
                
                for gx in 0..metrics.width {
                    let dx = glyph_x + gx as i32;
                    if dx < 0 || dx >= canvas.width as i32 { continue; }
