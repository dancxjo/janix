...
impl Texture {
    ...
    pub fn as_slice_mut(&mut self) -> &mut [u32] {
        if self.bpp != 4 { panic!("Texture is not 4bpp"); }
...
impl<'a> Canvas<'a> {
    ...
    pub fn blit(&mut self, src: &[u32], src_width: u32, src_height: u32, src_stride_pixels: u32, dst_x: i32, dst_y: i32) {
...
impl Atlas {
    ...
    pub fn pack(&mut self, w: u32, h: u32, pixels: &[u8]) -> Option<(u32, u32)> {
        ...
        for sy in 0..h {
            ...
            tex_buf[dst_off..dst_off + w as usize].copy_from_slice(&pixels[src_off..src_off + w as usize]);
        }
