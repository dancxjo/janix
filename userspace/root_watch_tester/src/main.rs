...
fn test_main() -> i32 {
    ...
    println!("\n--- Test 0b: Open with start_seq=0, verify not treated as latest ---");
    {
        ...
        if replay_watch != 0 {
            println!(
                "PASS: start_seq=0 watch opened successfully (will see historical events if any)"
            );
        }
    }
    ...
    println!("\n--- Test 0.5: Drain until EAGAIN contract ---");
    {
        ...
        if drain_watch != 0 {
            ...
            match watch_next(drain_watch, &mut buf) {
                ...
                Ok((len, _)) => {
                    println!(
                        "INFO: Initial watch_next returned {} bytes (non-fresh state)",
                        len
                    );
                }
                Err(e) => {
        ...
        match watch_next(watch_handle, &mut buf) {
            Ok((len, seq)) => {
                ...
                if len > 8 && buf[8] == OP_CREATE_NODE {
                    ...
                    println!(
                        "FAIL: Expected CREATE_NODE tag (0x01), got {:02x}",
                        buf.get(8).unwrap_or(&0)
                    );
                    failures += 1;
                }
            ...
            match watch_next(watch_handle, &mut buf) {
                Ok((len, seq)) => {
                    ...
                    if len > 8 && buf[8] == OP_PUT_EDGE {
                        ...
                        println!(
                            "FAIL: Expected PUT_EDGE tag (0x02), got {:02x}",
                            buf.get(8).unwrap_or(&0)
                        );
                        failures += 1;
                    }
            ...
            match watch_next(watch_handle, &mut buf) {
                Ok((len, seq)) => {
                    ...
                    if len > 8 && buf[8] == OP_SET_PROP {
                        ...
                        println!(
                            "FAIL: Expected SET_PROP tag (0x03), got {:02x}",
                            buf.get(8).unwrap_or(&0)
                        );
                        failures += 1;
                    }
        ...
        if (res as isize) < 0 {
            ...
            match watch_next(watch_handle, &mut buf) {
                Ok((len, seq)) => {
                    ...
                    if validate_header(&buf[..len]) {
                        println!(
                            "PASS: APPLY_BATCH and single-op both produce consistent watch events"
                        );
                    } else {
    ...
    println!("\n--- Test 6: Two watchers with different filters ---");
    {
        ...
        if watch_a != 0 && watch_b != 0 {
            ...
            if a_saw_event && b_saw_event {
                ...
                println!(
                    "FAIL: Watch A saw={}, Watch B saw={}",
                    a_saw_event, b_saw_event
                );
                failures += 1;
            }
        } else {
