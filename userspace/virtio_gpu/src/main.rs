...
impl VirtioGpu {
    ...
    fn create_resource_2d(&mut self) -> Result<(), &'static str> {
        info!("VIRTIO_GPU: Creating 2D resource {}x{}", self.display_width, self.display_height);
    ...
    fn attach_backing(&mut self) -> Result<(), &'static str> {
        info!("VIRTIO_GPU: Attaching backing memory @ phys=0x{:x}", self.fb_phys);
    ...
    fn set_scanout(&mut self) -> Result<(), &'static str> {
        info!("VIRTIO_GPU: Setting scanout 0 to resource {}", self.resource_id);
