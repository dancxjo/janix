...
pub fn run_bios(sh: &Shell, qemu_flags: &str, iso_path: &Path) -> Result<()> {
    ...
    cmd!(
        sh,
        "qemu-system-x86_64 -M q35 -device virtio-vga-gl -display gtk,gl=on -serial stdio -cdrom {iso} -boot d -device virtio-net-pci,netdev=n0,mac=52:54:00:12:34:56,disable-legacy=on -netdev user,id=n0"
    )
    .args(&qemu_args)
    .run()?;
    Ok(())
...
pub fn run_hdd(sh: &Shell, arch: &str, qemu_flags: &str, hdd_path: &Path) -> Result<()> {
    ...
    match arch {
        "x86_64" => {
            // virtio-vga-gl enables virgl 3D acceleration
            // virtio-net-pci for network support
            cmd!(sh, "qemu-system-x86_64 -M q35 -device virtio-vga-gl -display gtk,gl=on -serial stdio -drive if=pflash,unit=0,format=raw,file={ovmf_code},readonly=on -drive if=pflash,unit=1,format=raw,file={ovmf_vars} -hda {hdd} -device virtio-net-pci,netdev=n0,mac=52:54:00:12:34:56,disable-legacy=on -netdev user,id=n0")
                .args(&qemu_args)
                .run()?;
        }
        "aarch64" => {
