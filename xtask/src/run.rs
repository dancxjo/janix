...
pub fn run(sh: &Shell, arch: &str, qemu_flags: &str, iso_path: &Path) -> Result<()> {
    ...
    match arch {
        "x86_64" => {
            cmd!(sh, "qemu-system-x86_64 -M q35 -device virtio-vga  -serial stdio -drive if=pflash,unit=0,format=raw,file={ovmf_code},readonly=on -drive if=pflash,unit=1,format=raw,file={ovmf_vars} -cdrom {iso} -drive if=none,id=hdd0,file=test_disk.img,format=raw -device virtio-blk-pci,drive=hdd0 -no-reboot -d int,cpu_reset -D qemu.log")
                .args(&qemu_args)
                .run()?;
        }
        "aarch64" => {
            cmd!(sh, "qemu-system-aarch64 -M virt -cpu cortex-a72 -serial stdio -semihosting -device ramfb -device qemu-xhci -device usb-kbd -device usb-mouse -drive if=pflash,unit=0,format=raw,file={ovmf_code},readonly=on -drive if=pflash,unit=1,format=raw,file={ovmf_vars} -cdrom {iso} -drive if=none,id=hdd0,file=test_disk.img,format=raw -device virtio-blk-pci,drive=hdd0")
                .args(&qemu_args)
                .run()?;
        }
        "riscv64" => {
            ...
            cmd!(sh, "qemu-system-riscv64 -blockdev node-name=pflash0,driver=file,read-only=on,filename={ovmf_code} -blockdev node-name=pflash1,driver=file,filename={ovmf_vars} -M virt,pflash0=pflash0,pflash1=pflash1 -cpu rv64 -m 2G -serial stdio -device ramfb -device qemu-xhci -device usb-kbd -device usb-mouse -drive file={iso},format=raw,if=none,id=drive0,readonly=on -device virtio-blk-device,drive=drive0 -drive if=none,id=hdd1,file=test_disk.img,format=raw -device virtio-blk-device,drive=hdd1")
                .args(&qemu_args)
                .run()?;
        }
        "loongarch64" => {
            cmd!(sh, "qemu-system-loongarch64 -M virt -cpu la464 -serial stdio -device ramfb -device qemu-xhci -device usb-kbd -device usb-mouse -drive if=pflash,unit=0,format=raw,file={ovmf_code},readonly=on -drive if=pflash,unit=1,format=raw,file={ovmf_vars} -cdrom {iso} -drive if=none,id=hdd0,file=test_disk.img,format=raw -device virtio-blk-pci,drive=hdd0")
                .args(&qemu_args)
                .run()?;
        }
...
fn ensure_test_disk(sh: &Shell) -> Result<()> {
    if !sh.path_exists("test_disk.img") {
        println!("Creating test_disk.img (64MB)...");
        cmd!(sh, "dd if=/dev/zero of=test_disk.img bs=1M count=64 status=none").run()?;
    }
    Ok(())
}
...
pub fn run_bios(sh: &Shell, qemu_flags: &str, iso_path: &Path) -> Result<()> {
    ...
    cmd!(
        sh,
        "qemu-system-x86_64 -M q35 -device virtio-vga  -serial stdio -cdrom {iso} -boot d -drive if=none,id=hdd0,file=test_disk.img,format=raw -device virtio-blk-pci,drive=hdd0"
    )
    .args(&qemu_args)
    .run()?;
    Ok(())
