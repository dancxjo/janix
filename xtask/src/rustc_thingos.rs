...
//!    `x86_64-unknown-thingos`. If available, `stage_rustc_for_iso()` stages
//!    it into the boot ISO at:
...
/// Generate a `config.toml` for `x.py` that builds a Linux-hosted stage-1
/// `rustc` which targets `x86_64-unknown-thingos`.
///
...
///
/// We intentionally keep Linux as the only host in this config.
///
/// Building a ThingOS-hosted stage-1 compiler currently relies on bootstrap
/// internals that have changed upstream and can leave the stage0 sysroot
/// without ThingOS `core/std` artifacts. Keeping ThingOS in `target` still
/// builds the libraries needed by the Linux-hosted cross-compiler.
///
...
# Host list is parameterized so we can run a safe Linux-hosted build first,
...
fn try_build_thingos_native_rustc(sh: &Shell, cwd: &Path, rust_src: &Path) -> Result<bool> {
...
/// Build a Linux-hosted stage-1 `rustc` and cache the result under
...
///
/// A ThingOS-native compiler may still be discovered from available build
/// artifacts and cached opportunistically for ISO staging.
///
