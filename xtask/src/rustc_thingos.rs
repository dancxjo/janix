...
fn bootstrap_config() -> String {
    ...
    // target spec.  The Rust bootstrap must therefore build `rustc_driver` as
    // an rlib (static archive) rather than a dylib, and link it statically
    // into the `rustc` binary.
    //
    // Bootstrap config levers used here:
    //   [rust]  rpath = false   – no rpath embedding; the binary must be
    //                             self-contained (all libs resolved at link-time).
    //
    // Required Rust-fork change (vendor/rust, branch thingos-patched)
    // ----------------------------------------------------------------
    // The upstream bootstrap code in
    //   src/bootstrap/src/core/build_steps/compile.rs
    // builds `rustc_driver` with `--crate-type dylib` unconditionally.  For
    // targets where `supports_dylib()` returns false (i.e., targets whose spec
    // has `"dynamic-linking": false`), the bootstrap must be patched to fall
    // back to `--crate-type rlib` and to link the `rustc` binary against that
    // rlib statically.  Until that fork patch lands this build step will fail
    // at the `rustc_driver` dylib stage; the target-spec flag at least makes
    // the failure message unambiguous and ensures cargo itself never attempts a
    // dylib for ThingOS user code.
    r#"change-id = "ignore"
